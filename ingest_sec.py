...
def get_quarterly_ytd(facts, tags):
    """Return quarterly YTD entries from 10-Q filings that have start+end dates."""
    for tag in tags:
        if tag in facts.get("us-gaap", {}):
            entries = facts["us-gaap"][tag]["units"].get("USD", [])
            quarterly = [
...
for ticker, cik in COMPANIES.items():
    ...
    for year in recent:
        annual_records.append({
            ...
            "sector":           sector,
            "fiscal_year":      year,
            "revenue":          rev.get(year),
            "net_income":       net_inc.get(year),
            "operating_income": op_inc.get(year),
    ...
    # Index by (fy, fp) for easy lookup
    q_rev_idx = {(e["fy"], e["fp"]): e for e in q_rev}
    q_ni_idx  = {(e["fy"], e["fp"]): e for e in q_ni}
    q_op_idx  = {(e["fy"], e["fp"]): e for e in q_op}

    all_keys = set(q_rev_idx) | set(q_ni_idx)
    recent_keys = [(fy, fp) for fy, fp in all_keys if fy >= 2019]

    for fy, fp in recent_keys:
    ...
    print(f"  {ticker}: {len(recent)} annual years, {len(recent_keys)} quarterly periods")
    time.sleep(0.15)

df_annual = pd.DataFrame(annual_records)
df_quarterly = pd.DataFrame(quarterly_records)

print(f"\nAnnual records: {len(df_annual)}")
print(f"Quarterly records: {len(df_quarterly)}")
...
print(f"Loaded {con.execute('SELECT COUNT(*) FROM raw.sec_financials').fetchone()[0]} rows into raw.sec_financials")
print(f"Loaded {con.execute('SELECT COUNT(*) FROM raw.sec_quarterly').fetchone()[0]} rows into raw.sec_quarterly")
con.close()
